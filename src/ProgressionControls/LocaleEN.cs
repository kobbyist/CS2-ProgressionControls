    ...
    public sealed class LocaleEN : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.XpPerResident)), "Population XP awarded per resident above the historical record. Choose 0 to 10 in 0.25 increments; 1.5 matches the nominal vanilla population rate. Click Apply custom rules to use the new value for future growth." },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.ApplyCustomRules)), "Validates and applies both advanced XP rule values together. Until clicked, the running city continues using the last applied configuration." },
